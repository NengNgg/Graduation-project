...
QRCode.prototype = {
    ...
    setupPositionProbePattern: function(a, b) {
        for (var d = -1; 7 >= d; d++) if (! ( - 1 >= a + d || this.moduleCount <= a + d)) for (var c = -1; 7 >= c; c++) - 1 >= b + c || this.moduleCount <= b + c || (this.modules[a + d][b + c] = 0 <= d && 6 >= d && (0 == c || 6 == c) || 0 <= c && 6 >= c && (0 == d || 6 == d) || 2 <= d && 4 >= d && 2 <= c && 4 >= c ? !0 : !1)
    },
    ...
    setupPositionAdjustPattern: function() {
        for (var a = QRUtil.getPatternPosition(this.typeNumber), b = 0; b < a.length; b++) for (var d = 0; d < a.length; d++) {
            var c = a[b],
                e = a[d];
            if (null == this.modules[c][e]) for (var f = -2; 2 >= f; f++) for (var h = -2; 2 >= h; h++) this.modules[c + f][e + h] = -2 == f || 2 == f || -2 == h || 2 == h || 0 == f && 0 == h ? !0 : !1
        }
    },
    ...
    mapData: function(a, b) {
        for (var d = -1,
                 ...
                 e = 7,
                 f = 0,
                 h = this.moduleCount - 1; 0 < h; h -= 2) for (6 == h && h--;;) {
            for (var g = 0; 2 > g; g++) if (null == this.modules[c][h - g]) {
                var k = !1;
...
QRCode.createData = function(a, b, d) {
    ...
    if (c.getLengthInBits() > 8 * a) throw Error("code length overflow. (" + c.getLengthInBits() + "\x3e" + 8 * a + ")");
...
QRCode.createBytes = function(a, b) {
    for (var d = 0,
             c = 0,
             e = 0,
             f = Array(b.length), h = Array(b.length), g = 0; g < b.length; g++) {
        var k = b[g].dataCount,
            m = b[g].totalCount - k,
            c = Math.max(c, k),
            e = Math.max(e, m);
        f[g] = Array(k);
        for (var l = 0; l < f[g].length; l++) f[g][l] = 255 & a.buffer[l + d];
        d += k;
        l = QRUtil.getErrorCorrectPolynomial(m);
        k = (new QRPolynomial(f[g], l.getLength() - 1)).mod(l);
        h[g] = Array(l.getLength() - 1);
        for (l = 0; l < h[g].length; l++) m = l + k.getLength() - h[g].length,
            h[g][l] = 0 <= m ? k.get(m) : 0
    }
    for (l = g = 0; l < b.length; l++) g += b[l].totalCount;
    d = Array(g);
    for (l = k = 0; l < c; l++) for (g = 0; g < b.length; g++) l < f[g].length && (d[k++] = f[g][l]);
    for (l = 0; l < e; l++) for (g = 0; g < b.length; g++) l < h[g].length && (d[k++] = h[g][l]);
    return d
};
for (var QRMode = {
        MODE_NUMBER: 1,
        ...
        MODE_KANJI: 8
         ...
         QRUtil = {
             ...
             getBCHTypeInfo: function(a) {
                 for (var b = a << 10; 0 <= QRUtil.getBCHDigit(b) - QRUtil.getBCHDigit(QRUtil.G15);) b ^= QRUtil.G15 << QRUtil.getBCHDigit(b) - QRUtil.getBCHDigit(QRUtil.G15);
             ...
             getErrorCorrectPolynomial: function(a) {
                 for (var b = new QRPolynomial([1], 0), d = 0; d < a; d++) b = b.multiply(new QRPolynomial([1, QRMath.gexp(d)], 0));
                 return b
             },
             ...
             getLostPoint: function(a) {
                 for (var b = a.getModuleCount(), d = 0, c = 0; c < b; c++) for (var e = 0; e < b; e++) {
                     for (var f = 0,
                              h = a.isDark(c, e), g = -1; 1 >= g; g++) if (! (0 > c + g || b <= c + g)) for (var k = -1; 1 >= k; k++) 0 > e + k || b <= e + k || 0 == g && 0 == k || h != a.isDark(c + g, e + k) || f++;
                     5 < f && (d += 3 + f - 5)
                 }
                 for (c = 0; c < b - 1; c++) for (e = 0; e < b - 1; e++) if (f = 0, a.isDark(c, e) && f++, a.isDark(c + 1, e) && f++, a.isDark(c, e + 1) && f++, a.isDark(c + 1, e + 1) && f++, 0 == f || 4 == f) d += 3;
...
QRPolynomial.prototype = {
    ...
    mod: function(a) {
        ...
        for (var b = QRMath.glog(this.get(0)) - QRMath.glog(a.get(0)), d = Array(this.getLength()), c = 0; c < this.getLength(); c++) d[c] = this.get(c);
...
QRRSBlock.getRSBlocks = function(a, b) {
    ...
    for (var c = d.length / 3,
             e = [], f = 0; f < c; f++) for (var h = d[3 * f + 0], g = d[3 * f + 1], k = d[3 * f + 2], m = 0; m < h; m++) e.push(new QRRSBlock(g, k));
    return e
};
...
var tctipUtil = {
        createElement: function(a, b, d) {
            ...
            for (var c in a)"style" == c ? b[c].cssText = a[c] : b[c] = a[c];
            return (d || document.body).appendChild(b)
        },
        getElementsByClassName: function(a, b) {
            b = b || document;
            if (b.getElementsByClassName) return b.getElementsByClassName(a);
            var d = [],
                ...
                h,
                g;
            for (g = h = 0; h < e; h++) f.test(c[h].className) && (d[g] = c[h], g++);
            return d
        },
        getTextKey: function() {
        ...
        getExplorer: function() {
            ...
            if (0 <= a.indexOf("msie")) return "ie";
            if (0 <= a.indexOf("firefox")) return "firefox";
            if (0 <= a.indexOf("chrome")) return "chrome";
            if (0 <= a.indexOf("opera")) return "opera";
            if (0 <= a.indexOf("safari")) return "safari"
        },
        addLoadEvent: function(a) {
            if ("function" != typeof window.onload) window.onload = a;
            else {
                var b = window.onload;
        ...
        isMouseLeaveOrEnter: function(a, b) {
            ...
            for (var d = a.relatedTarget ? a.relatedTarget: "mouseout" == a.type ? a.toElement: a.fromElement; d && d != b;) d = d.parentNode;
            return d != b
        },
        ...
        mergeArray: function(a, b, d) {
            for (var c in b) if (a.hasOwnProperty(c) || d) a[c] = b[c];
            return a
        },
        ...
        generateQR: function(a, b) {
            ...
            var d = function() {
                    ...
                    for (var f = a.width / b.getModuleCount(), h = a.height / b.getModuleCount(), g = 0; g < b.getModuleCount(); g++) {
                        ...
                        g.style.height = h + "px";
                        for (var m = 0; m < b.getModuleCount(); m++) k.insertCell( - 1),
                ...
                d = "canvas" == a.render ?
                    function() {
                        ...
                        d.width = a.width;
                        d.height = a.height;
                        for (var f = d.getContext("2d"), h = a.width / b.getModuleCount(), g = a.height / b.getModuleCount(), k = 0; k < b.getModuleCount(); k++) for (var m = 0; m < b.getModuleCount(); m++) {
    ...
    tctip = window.tctip || {
        ...
        generateMyRewardsMain: function() {
            ...
            a[tctipUtil.getTextKey()] = "\u4e86\u89e3\u66f4\u591a";
            tctipUtil.createElement(a, "a", b)
        },
        ...
        generateMyRewardsList: function() {
            ...
            this.myRewardsListUl = 5 <= tctip.myConfig.list.length ? tctipUtil.createElement({},
                "ul", this.myRewardsList) : tctipUtil.createElement({
                    className: "not-full"
                },
            ...
            for (var a = 0; 5 > a && tctip.myConfig.list.hasOwnProperty(a); a++) {
                ...
                "myR-on" == b.className && (this.currentLi = d, this.currentData = b);
                tctipUtil.createElement({
                        className: "png",
        ...
        generateMyRewardsDetail: function() {
            ...
            if (tctip.currentData.hasOwnProperty("qrimg")) {
            ...
            } else tctip.currentData.hasOwnProperty("account") && (tctipUtil.generateQR({
                ...
                a[tctipUtil.getTextKey()] = tctip.currentData.desc || tctip.currentData.name, b = tctipUtil.createElement(a, "p", this.myRewardsUbox), a = {},
                a[tctipUtil.getTextKey()] = tctip.currentData.account, tctipUtil.createElement({},
                "br", b), tctipUtil.createElement(a, "span", b))
        },
        ...
        init: function() {
            document.body ? (tctip.generateMyConfig(), tctip.loadCss(), tctip.generateMyRewards(), tctip.stat()) : setTimeout(tctip.init, 0)
        }
    };
tctip.init();